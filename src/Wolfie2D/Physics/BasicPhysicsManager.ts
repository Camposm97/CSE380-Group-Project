...
export default class BasicPhysicsManager extends PhysicsManager {
  ...
  update(deltaT: number): void {
    for (let node of this.dynamicNodes) {
      ...
      if (node.moving) {
        ...
        node.sweptRect.sweep(
          node._velocity,
          node.collisionShape.center,
          node.collisionShape.halfSize
        );
      } else {
      ...
      for (let other of this.staticNodes) {
        ...
        if (area > 0) {
          // We had a collision
          overlaps.push(
            new AreaCollision(area, collider, other, "GameNode", null)
          );
        }
      }
      ...
      for (let overlap of overlaps) {
        ...
        const hit = otherAABB.intersectSegment(
          node.collisionShape.center,
          node._velocity,
          node.collisionShape.halfSize
        );
      ...
      for (let overlap of overlaps) {
        ...
        if (overlap.type === "Tilemap" || overlap.other.isCollidable) {
          let collisionSide = overlap.collider.touchesAABBWithoutCorners(
            node.collisionShape.getBoundingRect()
          );
   ...
   */
  protected collideWithOrthogonalTilemap(
    node: Physical,
    tilemap: OrthogonalTilemap,
    overlaps: Array<AreaCollision>
  ): void {
