...
import { CoatColor } from "../scene/Constants";
...
export default class EntityManager {
    ...
    initGreenFlag(): void {
        ...
        this.greenFlag.position = new Vec2((coord[0] - 0.5) * 16, (coord[1] - 1.0) * 16)
        this.greenFlag.scale = new Vec2(0.5, 0.5);
        this.greenFlag.animation.play("IDLE");
        this.greenFlag.collisionShape = new AABB(this.greenFlag.position, new Vec2(0.8, 0.8))
    ...
    initWeapons(): void {
        ...
        for (let i = 0; i < data.numWeapons; i++) {
            ...
            let constr = RegistryManager.getRegistry('weaponTemplates').get(x.weaponType)
            let weaponType = new constr()
            weaponType.initialize(x)
            RegistryManager.getRegistry("weaponTypes").registerItem(x.name, weaponType)
        }
    }
    ...
    initPlayer(): void {
        let inv = new InventoryManager(this.scene, 2, 'inventorySlot', new Vec2(16,16), 4, 'slots1', 'items1')
            ...
            for (let bomb of this.bombs) {
                if (bomb && !bomb.isDestroyed) {
                    if (enemy && enemy.sweptRect && enemy.sweptRect.overlaps(bomb.collisionBoundary)) {
                        if ((<RobotAI>enemy._ai).listening) this.enemiesLeft--;
                        bomb.explode();
                        ...
                        this.bm.enemies = this.bm.enemies.filter((currentEnemy) => currentEnemy !== enemy._ai);
    ...
    bombCollision(): void {
        if (this.player.collisionShape.overlaps(this.nearestBomb.collisionBoundary)) {
    ...
    playerReachedGoal(): boolean {
        return this.enemies.length === 0 && this.player.collisionShape.overlaps(this.greenFlag.collisionShape)
    }
