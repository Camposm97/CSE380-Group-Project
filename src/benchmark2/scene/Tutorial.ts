...
const t1_2 = 'As McBendojee gets closer to the bombs, his coat color will change. '
...
const t1_12 = 'If you are 1 space away from the bomb and facing it, you can place a flag on by pressing "f". '
// {insert Red Flag Sprite here}'
const t1_13 = 'This won\'t deactivate the bomb, but know you know exactly which space it is on. '
const t1_14 = 'The goal is to get McBendorjee to the Green Flag '
// {insert Green Flag Sprite here}.'
const t1_15 = 'If there are any blocks blocking McBendorjee, you can push them out of the way. '
// {Insert Block sprite here}'
const t1_16 = 'Just be careful to not to block your own path. Blocks do not activate the bombs. '
const t1_17 = 'If you feel you have gotten stuck at anypoint, press "p" to reset the room. '
...
const t2_3 = 'The robot walks in the oposite direction that McBendorjee walks. '
const t2_4 = 'If McBendorjee attacks the robot by pressing space or left clicking, it will freeze for a moment, '
const t2_5 = 'then will walk in the same direction McBendorjee walks. Hit the robot again to get him '
const t2_6 = 'back to his primary behavior. '
const t2_7 = 'The only way to kill the robot is to have him step on the bomb. '
const t2_8 = 'If the robot steps on the bomb, the bomb will go off, destroying the enemy and the bomb. '
const t2_9 = 'If a room has robots, they must be destroyed before stepping on the green flag. '
...
const t4_3 = 'This robot does not move but instead shoots projectiles down, right, up and left. If you hit '
const t4_4 = 'the robot statue, it will move back a couple spaces and be frozen. Just like the mouse, the player '
const t4_5 = 'can push the robot statue while it\'s frozen. The statue will resume shooting projectiles once it unfreezes. '
...
export class Tutorial1_1 extends Scene {
    ...
    startScene(): void {
        ...
        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y-30), t1_10).fontSize = 18
        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y-5), t1_11 + t1_12).fontSize = 18
        ...
        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y+60), t1_13 + t1_14).fontSize = 18
        ...
        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y+135), t1_15).fontSize = 18
...
export class Tutorial1_2 extends Scene {
    ...
    startScene(): void {
        ...
        let blueRobotSprite1 = this.add.animatedSprite('blue_robot', TUTORIAL_LAYER)
        blueRobotSprite1.animation.play(RobotAnimations.IDLE, true)
        blueRobotSprite1.position = new Vec2(c.x,c.y-280)
        blueRobotSprite1.scale = new Vec2(.4,.4)

        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y-210), t2_2 + t2_3).fontSize = 24
        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y-170), t2_4).fontSize = 24

        let blueRobotSprite2 = this.add.animatedSprite('blue_robot', TUTORIAL_LAYER)
        blueRobotSprite2.animation.play(RobotAnimations.FROZEN, true)
        blueRobotSprite2.position = new Vec2(c.x,c.y-90)
        blueRobotSprite2.scale = new Vec2(.4,.4)
        
        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y-20), t2_5).fontSize = 24
        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y+20), t2_6).fontSize = 24
        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y+60), t2_7).fontSize = 24
...
export class Tutorial1_3 extends Scene {
    ...
    startScene(): void {
        ...
        let blueRobotSprite1 = this.add.animatedSprite('blue_mouse', TUTORIAL_LAYER)
        blueRobotSprite1.animation.play(RobotMouseAnimations.IDLE, true)
        blueRobotSprite1.position = new Vec2(c.x,c.y-280)
        blueRobotSprite1.scale = new Vec2(.6,.6)

        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y-210), t3_2).fontSize = 24
        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y-170), t3_3).fontSize = 24
        
        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y-130), t3_4).fontSize = 24
        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y-90), t3_5).fontSize = 24
...
export class Tutorial1_4 extends Scene {
    ...
    startScene(): void {
        ...
        let blueRobotSprite1 = this.add.animatedSprite('blue_statue', TUTORIAL_LAYER)
        blueRobotSprite1.animation.play(RobotStatueAnimations.IDLE, true)
        blueRobotSprite1.position = new Vec2(c.x,c.y-280)
        blueRobotSprite1.scale = new Vec2(.4,.4)

        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y-210), t4_2).fontSize = 22
        initLabel(this, TUTORIAL_LAYER, new Vec2(c.x,c.y-170), t4_3).fontSize = 22
