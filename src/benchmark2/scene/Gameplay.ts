...
export default class Gameplay extends Scene {
    ...
    loadScene(){
        ...
        this.load.tilemap("level", "res/tilemaps/lava_level.json"); // Load my tile map
        ...
        while(this.receiver.hasNextEvent()) {
            ...
            if(event.isType("enemyDied")){
                ...
                if (this.battleManager.enemies.length === 0) {
                    ...
                    this.sceneManager.changeToScene(GameOver, {win: true, timeLeft: this.scoreTimer.getTimeLeftInSeconds()})
                }
            }
    ...
    initializeEnemies(){
        ...
        let actionsGun = [
            new Retreat(1, [Statuses.LOW_HEALTH, Statuses.CAN_RETREAT], [Statuses.REACHED_GOAL], {retreatDistance: 150}),
            new Berserk(2, [Statuses.LOW_HEALTH, Statuses.CAN_BERSERK], [Statuses.REACHED_GOAL], {}),
            new AttackAction(4, [Statuses.IN_RANGE], [Statuses.REACHED_GOAL]), 
            new Move(3, [], [Statuses.IN_RANGE], {inRange: 100})
        ];
        let actionsKnife = [
            new Berserk(1, [Statuses.LOW_HEALTH, Statuses.CAN_BERSERK], [Statuses.REACHED_GOAL], {}),
            new Move(2, [], [Statuses.IN_RANGE], {inRange: 20}), 
            new AttackAction(3, [Statuses.IN_RANGE], [Statuses.REACHED_GOAL]), 
            new Retreat(4, [Statuses.LOW_HEALTH, Statuses.CAN_RETREAT], [Statuses.REACHED_GOAL], {retreatDistance: 150}),
        ];
        let actionsCustom1 = [
            new AttackAction(1, [Statuses.IN_RANGE], [Statuses.REACHED_GOAL]), 
            new Move(2, [], [Statuses.IN_RANGE], {inRange: 100}),
            new Retreat(3, [Statuses.LOW_HEALTH, Statuses.CAN_RETREAT], [Statuses.REACHED_GOAL], {retreatDistance: 150}),
            new Berserk(4, [Statuses.CAN_BERSERK], [Statuses.REACHED_GOAL], {}),
        ];
        let actionsCustom2 = [
            new Berserk(1, [Statuses.CAN_BERSERK], [Statuses.REACHED_GOAL], {}),
            new Retreat(2, [Statuses.LOW_HEALTH, Statuses.CAN_RETREAT], [Statuses.REACHED_GOAL], {retreatDistance: 150}),
            new AttackAction(3, [Statuses.IN_RANGE], [Statuses.REACHED_GOAL]), 
            new Move(4, [], [Statuses.IN_RANGE], {inRange: 100})
        ];

        let gunPlans = this.generateGoapPlans(actionsGun, [Statuses.IN_RANGE, Statuses.LOW_HEALTH, Statuses.CAN_BERSERK, Statuses.CAN_RETREAT], Statuses.REACHED_GOAL);
        let knifePlans = this.generateGoapPlans(actionsKnife, [Statuses.IN_RANGE, Statuses.LOW_HEALTH, Statuses.CAN_BERSERK, Statuses.CAN_RETREAT], Statuses.REACHED_GOAL);
        let customPlans1 = this.generateGoapPlans(actionsCustom1, [Statuses.IN_RANGE, Statuses.LOW_HEALTH, Statuses.CAN_BERSERK, Statuses.CAN_RETREAT], Statuses.REACHED_GOAL);
        let customPlans2 = this.generateGoapPlans(actionsCustom2, [Statuses.IN_RANGE, Statuses.LOW_HEALTH, Statuses.CAN_BERSERK, Statuses.CAN_RETREAT], Statuses.REACHED_GOAL);
        ...
        for(let i = 0; i < enemyData.numEnemies; i++){
            ...
            this.enemies[i].position.set(entity.position[0]/2, entity.position[1]/2);
            ...
            if(entity.route){
                entity.route = entity.route.map((index: number) => this.graph.getNodePosition(index));                
            }

            if(entity.guardPosition){
                entity.guardPosition = new Vec2(entity.guardPosition[0]/2, entity.guardPosition[1]/2);
            }
