...
import Alert from "./enemy_states/Alert";
import Active from "./enemy_states/Active";
import Guard from "./enemy_states/Guard";
import Patrol from "./enemy_states/Patrol";
...
export default class EnemyAI extends StateMachineGoapAI implements BattlerAI {
    ...
    damage(damage: number): void {
        ...
        if (this.health <= Math.floor(this.maxHealth/2)) {
            if (this.currentStatus.indexOf(Statuses.LOW_HEALTH) === -1){
                this.currentStatus.push(Statuses.LOW_HEALTH);
