...
export default class Active extends EnemyState {
    ...
    onEnter(options: Record<string, any>): void {
        ...
        this.parent.path = this.owner.getScene().getNavigationManager().getPath(Names.NAVMESH, this.owner.position, this.parent.lastPlayerPos, true);
    ...
    pickRetreatPath(pathToPlayer: NavigationPath){
        this.parent.retreatPath = this.owner.getScene().getNavigationManager().getPath(
            Names.NAVMESH, 
            this.owner.position, 
            (this.owner.position.clone().add(pathToPlayer.getMoveDirection(this.owner).scale(-100))), true);
    }
    ...
    update(deltaT: number): void {
        ...
        if (this.parent.playerPos !== null) {
            ...
            if (distance > this.parent.inRange) {
                let index = this.parent.currentStatus.indexOf(Statuses.IN_RANGE);
                if (index != -1) {
        ...
        if (result !== null) {
            ...
            if (nextAction.toString() === "(Retreat)"){
                // console.log("next action is retreat")
                let index = this.parent.currentStatus.indexOf(Statuses.CAN_RETREAT);
                if (index != -1) {
                    this.parent.currentStatus.splice(index, 1);
                }
            }
            if (nextAction.toString() === "(Berserk)"){
                // console.log('next action is berserk')
                let index = this.parent.currentStatus.indexOf(Statuses.CAN_BERSERK);
                if (index != -1) {
                    this.parent.currentStatus.splice(index, 1);
                }
            }
            ...
            if (!result.includes(Statuses.REACHED_GOAL)) {
