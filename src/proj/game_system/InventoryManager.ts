...
export default class InventoryManager {
  ...
  constructor(
    ...
    //     position.x + i * (this.slotSize.x + this.padding),
    //     position.y
    //   );
    // }

    // Add a rect for the selected slot
    // this.selectedSlot = <Rect>(
    //   scene.add.graphic(GraphicType.RECT, slotLayer, {
  ...
  addItem(item: Item): boolean {
    if (!this.items[this.currentSlot]) {
      ...
      //   item.moveSprite(
      //     new Vec2(
      //       this.position.x + this.currentSlot * (this.slotSize.x + this.padding),
      //       this.position.y
      //     ),
      //     this.itemLayer
      //   );
